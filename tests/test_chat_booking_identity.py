...
def test_complete_identity_with_confirmation_does_not_book(
    ...
    hold_service = chat_service.appointment_holds
    assert isinstance(hold_service, FakeAppointmentHoldService)
    hold_calls_before = len(hold_service.create_hold_calls)
    ...
    assert len(hold_service.create_hold_calls) == hold_calls_before
    assert result.intent == ChatReceptionistIntent.PATIENT_IDENTITY_COMPLETE
    assert "later step" in result.reply.lower()
    assert result.conversation.conversation_metadata["chat_context"]["patient_identity"] == {
        "full_name": "Jane Doe",
        "date_of_birth": "1990-05-15",
